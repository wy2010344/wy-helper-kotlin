 ...
 * @param velocityWhenZta1Plus zta>=1 时是否计算速度（停止判定需要，纯位移场景可省）
 */
fun springBase(
    elapsedTime: Float,
    deltaX: Float,
    ...
    zta: Float = 1f,
    omega0: Float = 20f,
    velocityWhenZta1Plus: Boolean = false,
): SpringOutValue {
    val t = elapsedTime / 1000f
    val v0 = -initialVelocity * 1000f
    if (zta < 1f) {
        // 欠阻尼：指数包络 × 衰减振荡
        val omegaD = omega0 * sqrt(1f - zta * zta)
        val cosCoeff = deltaX
        val sinCoeff = (v0 + zta * omega0 * deltaX) / omegaD
        val cos1 = cos(omegaD * t)
        val sin1 = sin(omegaD * t)
        val envelope = exp(-zta * omega0 * t)
        val displacement = envelope * (cosCoeff * cos1 + sinCoeff * sin1)
        val velocity = displacement * -omega0 * zta +
        ...
        val velocity = if (velocityWhenZta1Plus) {
            envelope * ((coeffA + coeffB * t) * -omega0 + coeffB)
        } else 0f
        return SpringOutValue(displacement, velocity)
    } else {
        // 过阻尼：两个衰减指数项叠加
        val cext = omega0 * sqrt(zta * zta - 1f)
        val gammaPlus = -zta * omega0 + cext
        val gammaMinus = -zta * omega0 - cext
...
fun getZtaAndOmega0From(stiffness: Float, damping: Float, mass: Float): SpringBaseArg =
    SpringBaseArg(
        ...
        zta = damping / (2f * sqrt(stiffness * mass)),
    )
...
fun spring(arg: SpringAnimationArg = SpringAnimationArg()): DeltaXAnimateConfig = { deltaX ->
    animationTime { diffTime, setDisplacement ->
        val out = springBase(
            ...
            arg.config.zta, arg.config.omega0, true,
        )
